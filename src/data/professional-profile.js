...
I am particularly interested in helping organizations improve inefficient processes, automate repetitive work, improve communication, and turn practical business ideas into working software.`,
  ...
  projects: [
    ...
    },
    {
      ...
      status: 'Pilot',
      description:
        'A multi-tenant communication and management platform for residential communities and property managers. It reduces administrative work, improves communication, tracks maintenance concerns, manages announcements and documents, supports facility bookings and events, improves community engagement, and creates opportunities for local-business partnerships. It can be adapted for retirement residences, nonprofit housing, housing cooperatives, churches, sports associations, community organizations, business parks, and mixed-use developments.',
      technologies: [
        'Next.js',
        'React',
        'Tailwind CSS',
  ...
  availability: {
    openTo: ['Full-time employment', 'Contract work', 'Project-based custom software'],
    location: 'Victoria, British Columbia, Canada',
    remote: 'Remote, hybrid, or on-site in Greater Victoria.',
    note: 'Ask about current availability through the contact page.',
  },
  behaviorRules: [
...
/**
 * Deterministic source catalog. Sources are selected on the server by keyword
 * match rather than generated by the model, so a link is never invented.
 */
export const SOURCE_CATALOG = [
  {
    id: 'community-hive',
    title: 'Community Hive case study',
    url: '/case-studies/community-hive',
    keywords: ['community hive', 'strata', 'hoa', 'property manage', 'resident', 'council', 'tenant'],
  },
  {
    id: 'projects',
    title: 'Projects',
    url: '/projects',
    keywords: ['project', 'accesslens', 'lipsync', 'portfolio', 'built', 'map', 'accessibility platform'],
  },
  {
    id: 'case-studies',
    title: 'Case studies',
    url: '/case-studies',
    keywords: ['case study', 'case studies', 'outcome', 'architecture', 'result'],
  },
  {
    id: 'custom-software',
    title: 'Custom software services',
    url: '/custom-software',
    keywords: ['custom software', 'quote', 'consult', 'business problem', 'automate', 'workflow', 'cost', 'hire you to build'],
  },
  {
    id: 'about',
    title: 'About Dan',
    url: '/about',
    keywords: ['about', 'background', 'experience', 'career', 'years', 'who is', 'disability', 'cerebral palsy'],
  },
  {
    id: 'skills',
    title: 'Skills and tools',
    url: '/skillstools',
    keywords: ['skill', 'stack', 'technology', 'next.js', 'react', 'node', 'mongodb', 'typescript', 'language'],
  },
  {
    id: 'contact',
    title: 'Contact Dan',
    url: '/contact',
    keywords: ['contact', 'hire', 'available', 'availability', 'rate', 'email', 'reach'],
  },
];

/**
 * Pick up to `limit` relevant sources for a question/answer pair.
 */
export function selectSources(text, limit = 3) {
  const haystack = (text || '').toLowerCase();
  const scored = SOURCE_CATALOG.map((source) => ({
    source,
    score: source.keywords.reduce((total, kw) => (haystack.includes(kw) ? total + 1 : total), 0),
  }))
    .filter((entry) => entry.score > 0)
    .sort((a, b) => b.score - a.score)
    .slice(0, limit)
    .map((entry) => ({ title: entry.source.title, url: entry.source.url }));
...
export const QUICK_PROMPTS = [
  {
    id: 'closest-project',
    label: 'Which of Dan’s projects is closest to my need?',
    text: 'Which of Dan’s projects is closest to my need? I want to reduce admin work and improve communication.',
  },
  {
    id: 'nextjs-experience',
    label: 'What is his Next.js experience?',
    text: 'What is Dan’s Next.js experience, and where has he used it in production?',
  },
  {
    id: 'improve-workflow',
    label: 'How could he improve our workflow?',
    text: 'How could Dan improve our workflow if a lot of our information lives in email and spreadsheets?',
  },
  {
    id: 'availability',
    label: 'What work is he available for?',
    text: 'What kind of work is Dan available for right now?',
  },
];
...
Open to: ${PROFESSIONAL_PROFILE.availability.openTo.join(', ')}
Based in: ${PROFESSIONAL_PROFILE.availability.location}
Work arrangement: ${PROFESSIONAL_PROFILE.availability.remote}
Note: ${PROFESSIONAL_PROFILE.availability.note}
